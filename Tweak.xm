...
static int hooked_ptrace(int request,
                         pid_t pid,
                         caddr_t addr,
                         int data) {
    if (request == 31) {
...
static int (*orig_sysctl)(int *, u_int, void *,
                          size_t *, void *, size_t);

static int hooked_sysctl(int *name,
                         u_int namelen,
                         void *info,
                         size_t *infosize,
                         void *newinfo,
                         size_t newinfosize) {
    int ret = orig_sysctl(name, namelen, info,
                          infosize, newinfo, newinfosize);
    if (namelen >= 4 &&
        name[0] == CTL_KERN &&
        name[1] == KERN_PROC &&
...
static void installBypassHooks(void) {
    ...
    NSLog(@"[AXIOM] bypass hooks live — "
          @"ptrace/sysctl/getppid patched");
}
...
- (void)buildUI {
  ...
  forControlEvents:UIControlEventTouchUpInside];
    ...
    // Shadow host (separate so cornerRadius doesn't clip shadow)
